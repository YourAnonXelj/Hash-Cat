...
def crack(hashvalue):
    ...
    if len(hashvalue) == 32:
        if not file:
            print ('%s Tipus de hash --> MD5' % info)
        for api in md5:
            r = api(hashvalue, 'md5')
            if r:
                return r
        ...
        if not file:
            print ('%s Tipus de hash --> SHA1' % info)
        for api in sha1:
        ...
        if not file:
            print ('%s Tipus de hash --> SHA-256' % info)
        for api in sha256:
        ...
        if not file:
            print ('%s Tipus de hash --> SHA-384' % info)
        for api in sha384:
        ...
        if not file:
            print ('%s Tipus de hash --> SHA-512' % info)
        for api in sha512:
        ...
        if not file:
            print ('%s Aquest tipus de hash no és compatible.' % bad)
            quit()
        else:
            return False
...
def grepper(directory):
    ...
    print ('%s   %s.txt' % (info, directory.split('/')[-1]))
...
def miner(file):
    ...
    for line in lines:
        ...
        if matches:
            for match in matches:
                found.add(match)
    print ('%s Hashes trobades: %i' % (info, len(found)))
    ...
    for i, _ in enumerate(concurrent.futures.as_completed(futures)):
        if i + 1 == len(found) or (i + 1) % thread_count == 0:
            print('%s Progrés: %i/%i' % (info, i + 1, len(found)), end='\r')
...
def single(args):
    ...
    if result:
        ...
        print ('%s El hash no s,ha trobat en cap base de dades.' % bad)
...
if directory:
    ...
    print ('%s resultats guardats-%s' % (info, file.split('/')[-1]))
